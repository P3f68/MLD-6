...
partitionDisk()
{
	...
	if [ $(($(grep " $disk$" /proc/partitions | sed "s/.* \(.*\) .*/\1/")/1024/1024/1024)) -ge 2 ]; then #"
		if [ -e /sbin/sgdisk ]; then
			...
			lock apt-get -y install gdisk && gdisk=1
		fi
	fi
	...
	case "$parttype" in
		...
		activate)
			if [ "$gdisk" = "1" ]; then
				ls /dev/$disk?* | while read dev; do
					partition=$(echo $dev | sed "s/.*\([0-9]\)/\1/") #"
					sgdisk -A $partition:clear:2 /dev/$disk
				done
				partition=$(echo $system | sed "s/.*\([0-9]\)/\1/") #"
				sgdisk -A $partition:set:2 /dev/$disk
			else
				ls /dev/$disk?* | while read dev; do
...
copySystem()
{
	...
	rm -f var/spool/bootstep etc/udev/rules.d/00-no-auto-mount.rules
...
writeBootFilesBpi()
{
	...
	dd if=/boot/u-boot.bin of=/dev/$(echo $boot | sed "s/p\?.$//") bs=1024 seek=8
	); check_status
	return $?
}
...
#	ask "$(gt 'install MLD on selected disk ?')" || exit
	...
	if [ "$INSTALL_COPY_BOOT" = 1 -a -n "$INSTALL_BOOT_PARTITION" -a -n "$INSTALL_SYSTEM_PARTITION" ] || [ -n "$INSTALL_BOOTBLOCK_HDD" -a -n "$INSTALL_SYSTEM_PARTITION" ]; then
		INSTALL_BOOTBLOCK_HDD=${INSTALL_BOOTBLOCK_HDD%%:*}
		partitionDisk activate $INSTALL_BOOTBLOCK_HDD "$INSTALL_SYSTEM_PARTITION" || exit 1
